...
template.innerHTML = `
    <link rel="stylesheet" type="text/css" href="../game-board/game-board-style.css">
...
class GameBoard extends HTMLElement{
    constructor() {
        ...
        this.board = this.shadowRoot.querySelectorAll(".sliding-puzzle")[0];
        this.puzzleHeight = this.shadowRoot.querySelector(".sliding-puzzle").clientHeight;
        this.shuffleEl = this.shadowRoot.getElementById("shuffle");
        this.giveUp = this.shadowRoot.getElementById("give-up");
        this.size;
        this.baseDistance;
        this.parts = [];
    ...
    playSetup = () => {
        const playBtn = this.shadowRoot.querySelectorAll(".play")[0];
        const winLabel = this.shadowRoot.querySelectorAll(".win-label")[0];
        const levels = this.shadowRoot.querySelectorAll(".level")[0];
        const separator = [...this.shadowRoot.querySelectorAll(".separator")];
        const level = [...this.shadowRoot.querySelectorAll(".option")];
    ...
    endGame = (isWin) => {
        const partsElements = this.shadowRoot.querySelectorAll('.part');
        const winLabel = this.shadowRoot.querySelectorAll(".win-label")[0];
        const separator = [...this.shadowRoot.querySelectorAll(".separator")];
        const levels = this.shadowRoot.querySelectorAll(".level")[0];
        const playBtn = this.shadowRoot.querySelectorAll(".play")[0];
    ...
    movePart = (part) => {
        ...
        this.parts[this.size * this.size - 1].position = this.parts[partNumber].position;
    ...
    checkSolution = () => {
        if (this.parts[this.size * this.size - 1].position !== this.size * this.size) return false;
    ...
    shuffleLoop = () => {
        let partsElements = this.shadowRoot.querySelectorAll('.part');
        let emptyPosition = this.parts[this.size * this.size - 1].position;
    ...
    connectedCallback() {
        ...
        this.giveUp.addEventListener('click', () => { this.endGame(false) });
        this.shuffleEl.classList.add("hidden");
        this.giveUp.classList.add("hidden");
    }
    ...
    disconnectedCallBack() {
        ...
        this.giveUp.removeEventListener('click', () => { this.endGame(false) });
    }
}
